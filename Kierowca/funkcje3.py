...
def say_hello3(tekst):
    print(f'Witaj {tekst} ! ')


def prepare_group_of_maps3(lista_uzytkownikow: list) -> None:
...
def show_user_list3(lista_uzytkownikow: list) -> None:
    for user in lista_uzytkownikow[0:]:
        print(get_user3(user["name"], user['kierowca'], user["miejsce"]))


def get_user3(nick, kierowca, miejsce):
    return f'Twój znajomy {nick} dodał nowego kierowcę: {kierowca} w miejscowosci:{miejsce}, ' \
           f'wspolrzedne {get_coooridinates3(miejsce)}'


def get_coooridinates3(nazwa_miejscowosci: str) -> list:
    # 1.pobierz strone
...
def prepare_map3(nazwa_miejscowosci: list, tresc_popup: str, id: int) -> None:
...
def prepare_single_map3(lista_wspolrzednych: list[int], tekst: list[str]) -> None:
...
def delete_user3(user_list2: list) -> list:
...
def my_fellow3(name: str, miejsce: str, kierowca: list[str]) -> None:
    print('---------------------------------------------')
    print(f'         {name}                            ')
    print('---------------------------------------------')
...
def view_user3(user_list2: list[dict]) -> None:
